...
Image* CreateNewImage(Image* image, char* comment, int n)
{
    Image* outimage;
    int size, j;
    ...
    if (outimage->Type == GRAY)   size = image->Width * n * image->Height * n;
    else if (outimage->Type == COLOR) size = image->Width * n * image->Height * n * 3;

    outimage->data = (unsigned char*)malloc(size);
    outimage->Width = image->Width * n;
    outimage->Height = image->Height * n;
